#' Generate PI Maps
#'
#' Generate and save Person-Item maps for Rasch models.
#'
#' @param models Single CML model from eRm or named list of models
#' @param plot_path Path to folder where plots should be saved
#' @return NULL (saves plots to files)
#' @export
generate_pi_maps <- function(models, plot_path) {
...
#' Generate PW Maps
#'
#' Generate and save Person-Wright maps (item and person maps) for Rasch models.
#'
#' @param models Single CML model from eRm or named list of models
#' @param plot_path Path to folder where plots should be saved
#' @return NULL (saves plots to files)
#' @export
generate_pw_maps <- function(models, plot_path) {
  if (!inherits(models, "list")) {
    models <- list("model" = models)
  }
...
#' Generate Sampled Person-Item Maps
#'
#' Generate and save sampled person-item maps for Rasch models.
#'
#' @param models Single CML model from eRm or named list of models
#' @param plot_path Path to folder where plots should be saved
...
#' @return NULL (saves plots to files)
#' @export
generate_sampled_maps <- function(models, plot_path, seed = 42) {
  if (!inherits(models, "list")) {
    models <- list("model" = models)
  }
...
#' Extract Person Fit Statistics
#'
#' Extract person fit statistics from Rasch models.
#'
#' @param models Single CML model from eRm or named list of models
#' @param surveys Single survey data frame or named list of survey data
#' @param csv_path Path to folder where CSV files should be saved (optional)
#' @return List of data frames with person fit statistics
#' @export
extract_person_fit <- function(models, surveys, csv_path = NULL) {
  if (!inherits(models, "list")) {
    models <- list("model" = models)
    surveys <- list("survey" = surveys)
  }
  
  person_fit_results <- purrr::map2(models, surveys, ~ {
  ...
  if (!is.null(csv_path)) {
    purrr::walk2(person_fit_results, names(person_fit_results), ~ {
      .x %>% 
  ...
  return(person_fit_results)
}

#' Generate REBL Histograms
#'
#' Generate and save REBL score histograms.
#'
#' @param person_fit_data List of person fit data frames
#' @param plot_path Path to folder where plots should be saved
#' @return NULL (saves plots to files)
#' @export
generate_rebl_histograms <- function(person_fit_data, plot_path) {
  lower_limit <- purrr::map_dbl(person_fit_data, ~ {
    min(.x$rebl_score)
  }) %>% min()
  
  upper_limit <- purrr::map_dbl(person_fit_data, ~ {
    max(.x$rebl_score)
  }) %>% max()
  
  # Combined histogram with common x axis limits
  rebl_hists <- purrr::map2(person_fit_data, names(person_fit_data), ~ {
    .x %>% 
      as.data.frame() %>% 
  ...
  rebl_hists_indy <- purrr::map2(person_fit_data, names(person_fit_data), ~ {
    .x %>% 
      as.data.frame() %>% 
      ggplot2::ggplot(ggplot2::aes(x = rebl_score)) +
  ...
  purrr::walk2(rebl_hists_indy, names(person_fit_data), ~ {
    ggplot2::ggsave(
...
#' Generate ICC Plots
#'
#' Generate and save Item Characteristic Curve plots.
#'
#' @param models Single CML model from eRm or named list of models
#' @param plot_path Path to folder where plots should be saved
#' @return NULL (saves plots to files)
#' @export
generate_icc_plots <- function(models, plot_path) {
  if (!inherits(models, "list")) {
    models <- list("model" = models)
  }
...
#' Extract Item Fit Statistics
#'
#' Extract item fit statistics from Rasch models.
#'
#' @param models Single CML model from eRm or named list of models
#' @param surveys Single survey data frame or named list of survey data
#' @param rebl_items Vector of final REBL item names
#' @param csv_path Path to folder where CSV files should be saved (optional)
#' @return List of data frames with item fit statistics
#' @export
extract_item_fit <- function(models, surveys, rebl_items, csv_path = NULL) {
  if (!inherits(models, "list")) {
    models <- list("model" = models)
    surveys <- list("survey" = surveys)
  }
  
  item_fit_results <- purrr::map2(models, surveys, ~ {
    percent_eco_df <- .y %>%
  ...
  if (!is.null(csv_path)) {
    purrr::walk2(item_fit_results, names(item_fit_results), ~ {
      .x %>% 
...
#' with person and item fit statistics. Now supports both single inputs and lists.
#'
#' @param models Single CML model from eRm or named list of models
#' @param surveys Single survey data frame or named list of survey data
#' @param plot_path Path to folder where plots should be saved
...
#' )
#' 
#' # Example usage with single model/survey
#' results <- get_rasch_outputs(
#'   models = single_model,
